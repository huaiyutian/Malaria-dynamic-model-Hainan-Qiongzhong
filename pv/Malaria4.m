...
HE(1)=HI1(1);%+HI2(1)
HS2(1)=N(1)*y(2);
HS1(1)=N(1)-HI1(1)-HE(1)-HS2(1);%-HI2(1)
HR(1)=0;
HQ1(1)=0;
HQ2(1)=0;
HQ3(1)=0;
...
HI1(i)=HI1(i-1)+(3*qi1*HQ3(i-1)+ei1*HE(i-1)-HI1(i-1)*ps*ts*(1/8)-HI1(i-1)*ps*(1-ts)*i1i2  -  HI1(i-1)*(1-ps)*i1i2  +si2*lambda(i-1)*HS2(i-1)-HI1(i-1)*death);%si1*lambda(i-1)*HI2(i-1)+
if HI1(i) < 0
    HI1(i)=0;
end

HQ1(i)=HQ1(i-1)+ HS2(i-1)*iq1 -3*qi1*HQ1(i-1)-HQ1(i-1)*death;
if HQ1(i) < 0
    HQ1(i)=0;
end
HQ2(i)=HQ2(i-1)+3*qi1*HQ1(i-1)-3*qi1*HQ2(i-1)-HQ2(i-1)*death;
if HQ2(i) < 0
    HQ2(i)=0;
end
HQ3(i)=HQ3(i-1)+3*qi1*HQ2(i-1)-3*qi1*HQ3(i-1)-HQ3(i-1)*death;
if HQ3(i) < 0
   HQ3(i)=0;
end

HS2(i)=HS2(i-1) +( HI1(i-1)*(1-ps)*i1i2+ HI1(i-1)*ps*(1-ts)*i1i2) - HS2(i-1)*iq1  -HS2(i-1)*s2s1-si2*lambda(i-1)*HS2(i-1)-DrugT(i-1)-HS2(i-1)*death;%+HI2(i-1)*i2s2 -si3*lambda(i-1)*HS2(i-1)
if HS2(i) < 0
    HS2(i)=0;
end
...
N(i)=HS1(i-1)+HE(i-1)+HI1(i-1)+HQ1(i-1)+HQ2(i-1)+HQ3(i-1)+HS2(i-1)+HR(i-1);%+HI2(i-1)

%Mosquito dynamics
k(i)=k(i-1)+2*beta(i-1)*(HI1(i-1)+sf*HS2(i-1))*tau/N(i-1) - 2*tau*k(i-1);%+qf*HI2(i-1)
if k(i)<0;
    k(i)=0;
end
